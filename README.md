...
- **丢弃法杖 (徒手体术与隔空召回)**：将当前手持法杖作为物理实体丢弃至场景中（在大地图与小地图上打上专属法杖标记），诺艾尔隐去法杖并进入徒手体术战斗状态（普通攻击切换为挥拳肉搏打击，附带专属挥拳破空粒子与受击效果）；靠近法杖按交互键即可重新拾取，或随时在修改器顶栏/卡片中点击【✨ 召回】隔空收回手中。
...
## 🐛 问题反馈与日志提取 / Issues & Logs

遇到崩溃闪退、功能失效、数值异常或注入连接失败？欢迎前往 [GitHub Issues 提交页面](https://github.com/ZZCYUN/AICTrainer/issues/new) 反馈。

为了能以最快速度定位并修复问题，**请务必随反馈附带关键运行日志文件**（日志包含游戏内部 `error detected: xxxxxx` 报错代码、异常堆栈以及注入器加载详情）。

### 1. 关键日志提取指引（最重要）

游戏与修改器在运行过程中会生成两部分关键日志，请按以下路径提取并提供：

#### A. 游戏底层运行与崩溃日志（排查游戏闪退、报错弹窗与卡死必看）
- **一键直达路径**：按键盘快捷键 <kbd>Win</kbd> + <kbd>R</kbd> 打开「运行」窗口，输入以下路径并按回车：
  ```text
  %USERPROFILE%\AppData\LocalLow\NanameHacha\AliceInCradle
  ```
- **需要提取并上传的文件**：
  1. **`Player.log`**：当前（或最近一次）游戏运行的 Unity 引擎日志；若游戏刚刚发生闪退崩溃，同目录下的 **`Player-prev.log`** 即为崩溃发生时的现场日志。
  2. **`Logs/` 目录中的最新时间戳文件**（例如 `Logs/2026-09-11-22-34-08.txt`）：游戏每次启动都会生成独立报告，包含核心底层 `error detected: xxxxxx` 代码、地图加载流程与未捕获异常。

#### B. 修改器载荷与注入日志（排查注入失败、未连接或补丁未生效必看）
- **日志路径**：位于游戏安装根目录下（即 `AliceInCradle.exe` 同级目录）：
  - **`payload.log`**：记录 Mono 动态注入载荷加载、Harmony 补丁安装统计及 Local TCP IPC 服务启动状态。

---

### 2. Issue 提交格式与方法

前往 [新建 Issue 页面](https://github.com/ZZCYUN/AICTrainer/issues/new)，复制并填写以下信息：
...
### 问题描述
（详细描述遇到了什么异常，例如：在 xx 地图使用 xx 功能导致报错 / 游戏黑屏 / 角色无法移动）
...
### 附件日志文件（直接拖拽文件到此文本框上传）
- [ ] Player.log（或 Player-prev.log）
- [ ] NanameHacha/AliceInCradle/Logs/ 下的最新时间戳 .txt 日志
- [ ] 游戏根目录下的 payload.log（若涉及注入/连接异常）
```

> [!TIP]
> **如何上传日志文件**：可以直接将 `Player.log`、`.txt` 等日志文件**直接拖拽（Drag & Drop）**至 GitHub Issue 编辑框中，GitHub 会自动将其上传为附件。请切勿仅截图半截或粘贴数千行纯文本，以免信息截断或刷屏。
